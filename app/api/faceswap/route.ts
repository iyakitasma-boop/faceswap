...
export async function POST(req: NextRequest) {
  try {
    ...
    // Use lucataco/faceswap model on Replicate
    // source = face donor (whose face to use), target = base image (where to place the face)
    const replicateRes = await fetch('https://api.replicate.com/v1/predictions', {
      ...
      body: JSON.stringify({
        version: 'cff87316e31787df12002c9e20a78a017a36b65e36dde4e25a0cc4a7dd89939b',
        input: {
          swap_image: sourceBase64,   // The face to swap in
          target_image: targetBase64, // The image to receive the new face
        },
      }),
    });
